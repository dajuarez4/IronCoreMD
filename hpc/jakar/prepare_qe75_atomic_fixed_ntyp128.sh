#!/usr/bin/env bash
# Prepare a clean QE 7.5 source tree with the two private IronCoreMD patches.
# Usage:
#   bash prepare_qe75_atomic_fixed_ntyp128.sh /absolute/path/q-e-qe-7.5.tar.gz [source_parent]
set -euo pipefail
...
[[ ${#SOURCE_DIRS[@]} -eq 1 ]] || {
    echo "Expected exactly one top-level source directory in the archive." >&2
    exit 4
}
...
grep -Eq 'INTEGER,[[:space:]]*PARAMETER[[:space:]]*::[[:space:]]*ntypx[[:space:]]*=[[:space:]]*128' \
    "$SOURCE_DIR/Modules/parameters.f90" || {
        echo "ntypx=128 patch verification failed." >&2
        exit 5
    }
...
{
    echo "Quantum ESPRESSO 7.5 private IronCoreMD build for Jakar"
...
echo "Next: sbatch build_qe75_atomic_fixed_ntyp128_jakar.sbatch"
