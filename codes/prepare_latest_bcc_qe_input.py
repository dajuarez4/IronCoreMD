...
def build_collinear_random_test_input_from_frame(
    *,
    ...
    velocities_au, velocity_block, _ = finalize_velocities_for_qe(
        species_labels, velocities_au, masses_amu, target_temperature
    )
...
def prepare_bcc_qe_input(
    *,
    ...
    velocities_au = sample_maxwell_velocities_au(masses_amu, target_temperature, velocity_seed)
    if remove_com_drift and rescale_exact:
        velocities_au, velocity_block, measured_temperature = finalize_velocities_for_qe(
            labels, velocities_au, masses_amu, target_temperature
        )
    else:
        if remove_com_drift:
        ...
        if rescale_exact:
            velocities_au = rescale_to_temperature_au(
                velocities_au, masses_amu, target_temperature, remove_com=remove_com_drift
            )
        measured_temperature = temperature_from_velocities_au(
            velocities_au, masses_amu, remove_com=remove_com_drift
        )
