#!/usr/bin/env python3
"""Convert QE AIMD BCC NPZ archives into TDEP input folders."""
...
from tdep_common import default_dataset_dir, discover_bcc_npz_files
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert BCC QE NPZ files into TDEP folders.")
    parser.add_argument("npz", nargs="*", type=Path, help="NPZ files to convert. Default: all matching BCC NPZ files.")
    parser.add_argument(
        "--dataset-dir",
        type=Path,
        default=default_dataset_dir(),
        help="Directory containing the QE NPZ archives. Default: <repo>/dataset/bcc.",
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def bcc_primitive_uc(supercell_cell: np.ndarray, supercell: tuple[int, int, int]) -> tuple[np.ndarray, np.ndarray]:
    nx, ny, nz = supercell
    lengths = np.linalg.norm(np.asarray(supercell_cell, dtype=float), axis=1)
    a = float(np.mean(lengths / np.array([nx, ny, nz], dtype=float)))
    cell = np.array(
        [
            [-a / 2.0, a / 2.0, a / 2.0],
            [a / 2.0, -a / 2.0, a / 2.0],
    ...
    return cell, np.array([[0.0, 0.0, 0.0]], dtype=float)


def write_bcc_qpoints_dispersion(path: Path, points_per_path: int = 120) -> None:
    path.write_text(
        "\n".join(
            [
                "BCC",
...
def write_tdep_folder(
    ...
    uc_cell, uc_positions = bcc_primitive_uc(cell_ang, supercell)

    outdir.mkdir(parents=True, exist_ok=True)
    write_poscar(outdir / "infile.ucposcar", "Fe bcc primitive unit cell", uc_cell, uc_positions)
    write_poscar(outdir / "infile.ssposcar", "Fe bcc ideal supercell", cell_ang, ideal_frac)
    write_bcc_qpoints_dispersion(outdir / "infile.qpoints_dispersion")
    ...
    meta_lines = [
        ...
        f"skip: {skip}",
        f"every: {every}",
        f"dropped_invalid_frames: {int(len(all_indices[skip::every]) - len(selected)) if not keep_invalid else 0}",
    ]
...
def main() -> None:
    ...
    if not npz_files:
        npz_files = discover_bcc_npz_files(dataset_dir, None)
