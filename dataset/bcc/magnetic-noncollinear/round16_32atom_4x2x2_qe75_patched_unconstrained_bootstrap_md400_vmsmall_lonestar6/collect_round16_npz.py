#!/usr/bin/env python3
"""Collect Round-16 vm-small SCF/MD400 results into a lightweight safe NPZ archive."""
...
SCHEMA = "ironcoremd.qe.bcc32_patched_qe75_lambda0p100_md400_round16_vmsmall.v1"
NATOMS = 32
FLOAT = r"[-+]?(?:\d+\.\d*|\.\d+|\d+)(?:[EeDd][-+]?\d+)?"
SOURCES = (("bootstrap_unconstrained_2e-2", "scf"),
           ("bootstrap_l0p001_1e-2", "scf"),
           ("stage1_l0p005_4e-3", "scf"),
           ("stage2_l0p020_1e-3", "scf"),
           ("stage3_l0p100_3e-4", "scf"),
           ("md_400steps", "md"))
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", type=Path, default=ROOT / "round16_results_light.npz")
    ...
    md_states = [status[i * len(SOURCES) + len(SOURCES) - 1] for i in range(len(cases))]
    ...
    support = [path for path in (ROOT / "README.md", ROOT / "case_manifest.csv", ROOT / "velocity_summary.json",
                                  ...
                                  ROOT / "run_round16_lonestar6.sbatch", ROOT / "check_round16.sh",
                                  ROOT / "plot_round16.sh", ROOT / "generate_round16.py") if path.is_file()]
