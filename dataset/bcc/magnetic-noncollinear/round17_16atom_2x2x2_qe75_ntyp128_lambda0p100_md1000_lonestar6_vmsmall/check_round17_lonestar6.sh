...
if [[ -n "$latest" ]]; then
    ...
    } END {if(n>=16){s=0;for(i=0;i<16;i++)s+=v[i];printf "%.6f",s/16}}' "$latest")
    [[ -n "$value" ]] && moment=$value
    value=$(awk '/^[[:space:]]*magnetization[[:space:]]*:/ {
        slot=(n%16); v[slot]=sqrt($3*$3+$4*$4+$5*$5); n++;
        if(n%16==0){s=0;for(i=0;i<16;i++)s+=v[i]; frame++; avg[frame]=s/16}
