#!/usr/bin/env python3
"""Render the partial Round-17 LS6 structure/magnetization dashboard GIF."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-round17-dashboard")
...
DEFAULT_PNG = ROOT / "round17_vmsmall_component_dashboard_final.png"
DEFAULT_GIF = ROOT / "round17_vmsmall_component_dashboard.gif"
NATOMS = 16
...
VISUAL_ORIGIN_FRACTIONAL = np.asarray([0.125, 0.125, 0.125], dtype=float)
...
def scalar_values(pattern: str, text: str) -> np.ndarray:
    return np.asarray([number(value) for value in re.findall(pattern, text, re.I)], dtype=float)
...
def ideal_bcc_2x2x2() -> np.ndarray:
    return np.asarray(
        [
            [0.00, 0.00, 0.00], [0.25, 0.25, 0.25],
            [0.00, 0.50, 0.00], [0.25, 0.75, 0.25],
            [0.50, 0.00, 0.00], [0.75, 0.25, 0.25],
            [0.50, 0.50, 0.00], [0.75, 0.75, 0.25],
            [0.00, 0.00, 0.50], [0.25, 0.25, 0.75],
...
def match_ideal_sites(
    positions: np.ndarray, ideal_sites: np.ndarray, cell: np.ndarray
) -> np.ndarray:
    """Return ideal sites in atom order using periodic nearest neighbors."""
    delta = positions[:, None, :] - ideal_sites[None, :, :]
    delta -= np.rint(delta)
    squared_distance = np.sum((delta @ cell) ** 2, axis=2)
    assignments = np.argmin(squared_distance, axis=1)
    if len(np.unique(assignments)) != len(positions):
        raise ValueError("Initial atoms do not map uniquely to the ideal BCC sites")
    return ideal_sites[assignments]
...
def main() -> int:
    ...
    temperature = scalar_values(rf"temperature\s*=\s*({FLOAT})\s*K", text)
    pressure = scalar_values(rf"(?:^|\s)P\s*=\s*({FLOAT})", text) / 10.0
    local_magnetization = vector_frames(text, "magnetization")

    # Each converged force/magnetic record precedes its ionic propagation.
    # Pair force record 1 with the input coordinates, then records 2--1000 with
    # the coordinates printed after dynamics steps 1--999.
    ...
    if frame_count < 1:
        raise ValueError("No converged Round-17 MD SCFs were found")
    ...
    ideal_sites = ideal_bcc_2x2x2()
    # The thermalized input is not ordered like the canonical BCC site list.
    # Match periodically before computing per-atom displacements.
    ideal = match_ideal_sites(initial_positions, ideal_sites, cell)
    ...
    figure.suptitle(
        "Round 17 Noncollinear BCC Fe AIMD — Lonestar6 vm-small",
        x=0.045, y=0.975, ha="left", fontsize=24, fontweight="bold", color=COLORS["navy"],
    )
    figure.text(
        0.047, 0.925,
        f"16 atoms / 16 spin species • {frame_count}/1000 converged MD configurations • measured local-spin vectors",
    ...
    def update(frame_index: int):
        ...
        display_fractional = current_fractional
        ...
        displayed_spins = 0.85 * current_local / maximum_local
        ...
        for atom_index, coordinate in enumerate(current_cartesian, 1):
            structure_axis.text(
                coordinate[0] + 0.05, coordinate[1] + 0.05, coordinate[2] + 0.05,
                str(atom_index), color=COLORS["navy"], fontsize=7.2, fontweight="bold",
            )
        ...
        status.set_text(
            f"Configuration {frame_index}/{frame_count - 1} (partial {frame_count}/1000 run)   •   t = {time_ps[frame_index]:.4f} ps\n"
    ...
    animation.save(
        ...
        writer=PillowWriter(
            fps=args.fps,
            metadata={"title": "Round 17 vm-small component magnetization dashboard", "artist": "IronCoreMD"},
        ),
        dpi=args.dpi,
    )
    plt.close(figure)
