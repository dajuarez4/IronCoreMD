...
mkdir -p .round12_plot

prepare_data() {
    local index=0 case_name folder output
    : > .round12_plot/files.txt
    while IFS=, read -r serial case_name rest; do
        ...
        index=$((index + 1))
        printf '%s\t%s\n' "$index" "$case_name" >> .round12_plot/files.txt
        : > ".round12_plot/accuracy_${index}.dat"
        ...
        awk '/temperature[[:space:]]*=/{n++; value[n]=$(NF-1); first=(n>100?n-99:1);sum=0;for(i=first;i<=n;i++)sum+=value[i];print n,value[n],sum/(n-first+1)}' "$output" > ".round12_plot/temp_${index}.dat"
...
plot ".round12_plot/temp_1.dat" u 1:2 w l lw 1 lc rgb "#b998ff" title "instantaneous", \
     ".round12_plot/temp_1.dat" u 1:3 w l lw 3 lc rgb "#9b00ff" title "trailing mean (up to 100)", \
...
set xrange [0:402]
set yrange [0:2.2]
plot ".round12_plot/moment_1.dat" u 1:2 w l lw 1 lc rgb "#b998ff" title "instantaneous", \
