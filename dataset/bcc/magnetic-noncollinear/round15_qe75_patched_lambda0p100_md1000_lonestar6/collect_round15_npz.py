#!/usr/bin/env python3
"""Collect Round-15 SCF/MD1000 results into a lightweight safe NPZ archive."""
...
SCHEMA = "ironcoremd.qe.bcc8_patched_qe75_lambda0p100_md1000_round15.v1"
NATOMS = 8
FLOAT = r"[-+]?(?:\d+\.\d*|\.\d+|\d+)(?:[EeDd][-+]?\d+)?"
SOURCES = (("stage1_4e-3", "scf"), ("stage2_1e-3", "scf"),
           ("stage3_3e-4", "scf"), ("md_1000steps", "md"))
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", type=Path, default=ROOT / "round15_results_light.npz")
    ...
    for case_index, case in enumerate(cases):
        ...
        for stem, kind in SOURCES:
            ...
            if kind == "md" and not out.is_file() and (ROOT / f"{stem}.out").is_file():
                out = ROOT / f"{stem}.out"
    ...
    support = [path for path in (ROOT / "README.md", ROOT / "case_manifest.csv", ROOT / "velocity_summary.json",
                                  ROOT / "run_round15_lonestar6.sbatch", ROOT / "check_round15.sh",
                                  ROOT / "plot_round15.sh", ROOT / "generate_round15.py") if path.is_file()]
