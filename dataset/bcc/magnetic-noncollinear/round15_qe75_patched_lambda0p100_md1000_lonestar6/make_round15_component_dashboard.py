#!/usr/bin/env python3
"""Render the Round-15 structure and component-magnetization dashboard GIF."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-round15-dashboard")
...
CASE = "a2p39_m2_lambda0p100_md1000"
DEFAULT_NPZ = ROOT / "round15_results_light.npz"
DEFAULT_INPUT = ROOT / "cases" / CASE / "md_1000steps.in"
DEFAULT_OUTPUT = ROOT / "md_1000steps.out"
DEFAULT_PNG = ROOT / "round15_component_magnetization_dashboard_final.png"
DEFAULT_GIF = ROOT / "round15_component_magnetization_dashboard.gif"
NATOMS = 8
...
def main() -> int:
    ...
    if frame_count != 1000:
        raise ValueError(f"Expected 1000 converged Round-15 MD SCFs, found {frame_count}")
    ...
    figure.suptitle(
        "Round 15 Noncollinear BCC Fe AIMD Dashboard",
    ...
    def update(frame_index: int):
        ...
        status.set_text(
            f"Configuration {frame_index}/{frame_count - 1}   •   t = {time_ps[frame_index]:.4f} ps\n"
    ...
    animation.save(
        ...
        writer=PillowWriter(
            fps=args.fps,
            metadata={"title": "Round 15 component magnetization dashboard", "artist": "IronCoreMD"},
        ),
        dpi=args.dpi,
    )
    plt.close(figure)
