...
vector_tail_mean() {
    awk -v key="$2" -v window="$3" '$0 ~ "^[[:space:]]*" key "[[:space:]]*:" {
        for(i=1;i<=NF;i++) if($i==":") {
            slot=n%8; value[slot]=sqrt($(i+1)^2+$(i+2)^2+$(i+3)^2); n++;
            if(n%8==0){sum=0; for(j=0;j<8;j++)sum+=value[j]; report++; average[report]=sum/8}
            break
        }
    } END {
...
while IFS=, read -r index case_name lattice temperature target seed l1 l2 l3 requested_steps velocity_seed requirement; do
    ...
    folder="cases/$case_name"; md="$folder/md_1000steps.out"
    # Accept the lightweight return layout where only the MD output was
    # copied into the round root instead of its original case directory.
    [[ -f "$md" ]] || md="md_1000steps.out"
...
if [[ -f round15_status.tsv ]]; then
    echo
    column -t -s $'\t' round15_status.tsv 2>/dev/null || cat round15_status.tsv
fi
