#!/usr/bin/env python3
"""Generate the 48-species Round-19 Jakar workflow from Round 17."""
...
ROUND17 = ROOT.parent / "round17_16atom_2x2x2_qe75_ntyp128_lambda0p100_md1000_lonestar6_vmsmall"
ROUND17_INPUT = (
    ROUND17 / "cases" / "a2p39_2x2x2_m2_16species_lambda0p100_md1000" / "md_1000steps.in"
)
PSEUDO = "Fe.pbe-spn-kjpaw_psl.1.0.0.UPF"
LABELS = [f"Fe{i:02d}" for i in range(1, 49)]
...
# The first 16 directions are exactly Round 17. Four additional eight-vector
# groups continue the common azimuthal rotations. Every group contains four
# antipodal pairs, so the target-vector sum is zero while all 48 directions
# remain distinct.
ROUND15_ANGLES = (
...
ANGLES = tuple(
    (theta, (phi + shift) % 360.0)
    for shift in (0.0, 47.0, 94.0, 141.0, 188.0, 235.0)
    for theta, phi in ROUND15_ANGLES
)
STAGES = (
...
def round17_state() -> tuple[np.ndarray, np.ndarray]:
    ...
    for tile in range(3):
        copy = positions.copy()
        copy[:, 0] = (copy[:, 0] + tile) / 3.0
        tiles.append(copy)
    tiled_positions = np.concatenate(tiles, axis=0) % 1.0
    # Preserve the Round-17 speed distribution without making the three tiles
    # exact dynamical clones. Row and Cartesian-axis permutations are
    # deterministic and norm preserving.
    middle_velocities = np.roll(velocities, 5, axis=0)[:, [2, 0, 1]]
    right_velocities = np.roll(velocities, 9, axis=0)[:, [1, 2, 0]]
    tiled_velocities = np.concatenate(
        (velocities, middle_velocities, right_velocities), axis=0
    )
...
def rescale_velocities(velocities: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
    ...
    return velocities, {
        ...
        "source": (
            "Round-17 16-atom velocity set; second and third tiles use "
            "deterministic row/axis permutations; COM removed and exactly rescaled"
        ),
    }
...
def shuffled_labels() -> list[str]:
    ...
    for start in (0, 16, 32):
        group = np.asarray(LABELS[start:start + 16], dtype=object)
        rng.shuffle(group)
        groups.append(group)
    return np.concatenate(groups).tolist()
...
def cards(
    ...
    return lines + [
        ...
        "14.340000000000 0.000000000000 0.000000000000",
...
def main() -> None:
    ...
    positions, raw_velocities = round17_state()
    velocities, velocity_summary = rescale_velocities(raw_velocities)
    labels = shuffled_labels()
    vectors = np.asarray([direction(*angle) for angle in ANGLES])
    magnetic_summary = {
        "unique_species": 48,
        "unique_directions": 48,
        "one_species_per_atom": True,
        "target_local_moment_Bohr": 2.0,
        "first_sixteen_source": "Round 17",
        "direction_groups": "Round-15 base angles with common azimuth shifts 0, 47, 94, 141, 188, and 235 degrees",
    ...
    structure_summary = {
        ...
        "construction": "Round-17 2x2x2 structure tiled three times along x",
        "cell_A": [14.34, 4.78, 4.78],
        "natoms": NATOMS,
    }
    ...
    prefix = "Fe_bcc48_r19_a2p39_m2_l0100_md1000"
    ...
    print("Generated Round 19: 48 atoms, 48 species, 48 directions, MD1000 for Jakar")
