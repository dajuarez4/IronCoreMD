#!/usr/bin/env python3
"""Generate the 32-species Round-18 Lonestar6 workflow from Round 17."""
...
LABELS = [f"Fe{i:02d}" for i in range(1, 33)]
CASE = "a2p39_4x2x2_m2_32species_lambda0p100_md1000"
NATOMS = 32
...
# The first 16 directions are exactly Round 17. Two additional eight-vector
# groups use common azimuthal rotations of 94 and 141 degrees relative to the
# Round-15 base set. Every group contains four antipodal pairs, so the target
# vector sum is zero while all 32 directions remain distinct.
ROUND15_ANGLES = (
...
ANGLES = tuple(
    ...
    for shift in (0.0, 47.0, 94.0, 141.0)
...
def round17_state() -> tuple[np.ndarray, np.ndarray]:
    ...
    second[:, 0] = 0.5 * (second[:, 0] + 1.0)
    tiled_positions = np.concatenate((first, second), axis=0) % 1.0
    # Preserve the Round-17 speed distribution without making the right tile
    # an exact dynamical clone of the left one. A row permutation plus a
    # cyclic Cartesian-axis permutation is deterministic and norm preserving.
    right_velocities = np.roll(velocities, 5, axis=0)[:, [2, 0, 1]]
    tiled_velocities = np.concatenate((velocities, right_velocities), axis=0)
...
def rescale_velocities(velocities: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
    ...
    return velocities, {
        ...
        "source": (
            "Round-17 16-atom velocity set; right tile uses a deterministic "
...
def shuffled_labels() -> list[str]:
    ...
    left = np.asarray(LABELS[:16], dtype=object)
    right = np.asarray(LABELS[16:], dtype=object)
    rng.shuffle(left)
    rng.shuffle(right)
...
def main() -> None:
    ...
    magnetic_summary = {
        ...
        "direction_groups": "Round-15 base angles with common azimuth shifts 0, 47, 94, and 141 degrees",
    ...
    structure_summary = {
        ...
        "construction": "Round-17 2x2x2 structure tiled once along x",
    ...
    prefix = "Fe_bcc32_r18_a2p39_m2_l0100_md1000"
    ...
    print("Generated Round 18: 32 atoms, 32 species, 32 directions, MD1000")
