...
while IFS=, read -r index case_name lattice temperature target seed l1 l2 l3 requested_steps velocity_seed requirement; do
    ...
    latest=""
    for stem in bootstrap_l0p001_1e-2 stage1_l0p005_4e-3 stage2_l0p020_1e-3 stage3_l0p100_3e-4; do
...
if [[ -f round13_status.tsv ]]; then
    echo
    column -t -s $'\t' round13_status.tsv 2>/dev/null || cat round13_status.tsv
fi
