#!/usr/bin/env python3
"""Collect Round-13 SCF/MD400 results into a lightweight safe NPZ archive."""
...
SCHEMA = "ironcoremd.qe.bcc32_patched_qe75_lambda0p100_md400_round13.v1"
NATOMS = 32
FLOAT = r"[-+]?(?:\d+\.\d*|\.\d+|\d+)(?:[EeDd][-+]?\d+)?"
SOURCES = (("bootstrap_l0p001_1e-2", "scf"),
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", type=Path, default=ROOT / "round13_results_light.npz")
    ...
    support = [path for path in (ROOT / "README.md", ROOT / "case_manifest.csv", ROOT / "velocity_summary.json",
                                  ...
                                  ROOT / "run_round13_lonestar6.sbatch", ROOT / "check_round13.sh",
                                  ROOT / "plot_round13.sh", ROOT / "generate_round13.py") if path.is_file()]
