#!/usr/bin/env python3
"""Generate the corrected 32-atom BCC 4x2x2 noncollinear Round-13 workflow."""
...
STAGES = (
    # name, lambda (Ry), conv_thr (Ry), maxstep, degauss (Ry)
    ("bootstrap_l0p001_1e-2", 0.001, 1.0e-2, 500, 0.050000),
    ("stage1_l0p005_4e-3", 0.005, 4.0e-3, 500, 0.050000),
    ("stage2_l0p020_1e-3", 0.020, 1.0e-3, 500, 0.025334),
    ("stage3_l0p100_3e-4", 0.100, 3.0e-4, 600, 0.025334),
)
CASE = "a2p39_4x2x2_m2_scrambled_bootstrap_lambda0p100_md400"
...
def system(lambda_value: float, degauss: float) -> list[str]:
    lines = [
...
def electrons(read_state: bool, threshold: float, maxstep: int) -> list[str]:
    return [
        ...
        "   mixing_beta=0.010d0", "   mixing_ndim=40",
...
def main() -> None:
    ...
    prefix = "Fe_bcc32_r13_a2p39_4x2x2_m2_l0100_md400"
    for stage_index, (stem, lambda_value, threshold, maxstep, degauss) in enumerate(STAGES):
    ...
    print("Generated one Round-13 corrected BCC 4x2x2 32-atom, 400-step noncollinear case")
